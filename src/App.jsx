...
function App() {
  ...
  const renderStudents = filteredStudents.map((student, index) => (
    <li key={index}>
  ...
  return (
    <div>
      ...
      <input type="number" value={minAge} onChange={handleMinAgeChange} /> 살 이상
      <ul>
        {{renderStudents}/* TODO: map을 사용하여 필터링된 학생들의 정보를 표시하세요. 동명이인은 없다고 가정합니다. ex) 홍길동 - Age: 24, Grade: A */}
      </ul>
    </div>
  );
}
